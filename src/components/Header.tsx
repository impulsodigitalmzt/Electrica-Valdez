    ...
    <>
      <div className="bg-primary text-primary-foreground">
        <div className="mx-auto flex min-h-9 max-w-7xl flex-wrap items-center justify-center gap-x-5 gap-y-1 px-3 py-1.5 text-center text-[11px] font-semibold sm:px-4 sm:text-xs md:text-sm lg:justify-between">
          ...
          <div className="hidden items-center gap-5 lg:flex">
            <a href={telHref(TEL_PRINCIPAL)}>Tel. {TEL_LINEA}</a>
            {MAIN_EMAIL ? <a href={`mailto:${MAIN_EMAIL}`}>{MAIN_EMAIL}</a> : null}
      ...
      <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center gap-2 px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3 lg:gap-7">
          ...
          <AppLink to="/" className="flex min-w-0 shrink-0 items-center gap-2" aria-label="Eléctrica Valdez inicio">
            <img
              src={LOGO_SRC}
              alt="Eléctrica Valdez"
              className="h-9 w-auto max-w-[9.5rem] object-contain object-left sm:h-11 sm:max-w-[12rem] md:h-12 md:max-w-[14rem]"
            />
          </AppLink>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-0.5 sm:gap-1">
            {variant === "inner" ? (
              <Button variant="ghost" className="hidden text-black sm:flex" onClick={() => navigate("/")}>
                Inicio
              </Button>
            ) : null}
            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-1 px-4">
              {NAV_ITEMS.map((item) => (
                <AppLink
                  key={item.id}
                  to={item.to}
                  className={`flex h-12 items-center gap-1 border-b-2 px-1 text-[11px] font-bold uppercase tracking-tight transition-colors hover:text-black xl:px-2 xl:text-sm xl:tracking-normal ${
                    "sale" in item && item.sale ? "border-sale text-sale" : "border-transparent"
                  }`}
                >
                  {item.label}
                  {item.id !== "ofertas" ? <ChevronDown className="hidden size-3 xl:block" /> : null}
                </AppLink>
              ))}
            </div>
          </nav>
      ...
      <Sheet open={menu} onOpenChange={setMenu}>
        ...
        >
          <SheetTitle className="sr-only">Menú de navegación Eléctrica Valdez</SheetTitle>
            ...
            <button type="button" onClick={() => ir("/")} className="block w-full text-left" aria-label="Ir al inicio">
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-end gap-3 p-4 pr-14">
                <span className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-white p-1.5 shadow-md">
                  <img src={LOGO_MARK} alt="" className="h-full w-auto object-contain" />
                </span>
                <div className="min-w-0 text-white">
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary">Bienvenido</p>
                  <p className="font-display text-lg font-extrabold leading-tight">Eléctrica Valdez</p>
          ...
          <div className="space-y-4 px-4 pb-10 pt-4">
            ...
            >
              <img src={IMAGEN_OFERTA} alt="" className="aspect-[2.2/1] w-full object-cover object-center" />
            ...
            <div className="rounded-[22px] bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <ul className="mt-4 space-y-3.5 text-sm text-muted-foreground">
                <li className="flex gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-[#eef2f7] text-black">
                    <Clock className="size-4" />
                  </span>
                  <span className="pt-1.5">{HORARIO}</span>
                </li>
                <li className="flex gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-[#eef2f7] text-black">
                    <MapPin className="size-4" />
                  </span>
                  <span className="pt-1.5">{COBERTURA}</span>
                </li>
                <li className="flex gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-[#eef2f7] text-black">
                    <Phone className="size-4" />
                  </span>
                  <a className="pt-1.5 hover:text-black" href={telHref(TEL_PRINCIPAL)}>
                    {TEL_LINEA}
                  </a>
                </li>
                {MAIN_EMAIL ? (
                  <li className="flex gap-3">
                    ...
                    <a className="break-all pt-1.5 hover:text-black" href={`mailto:${MAIN_EMAIL}`}>
                      {MAIN_EMAIL}
                    </a>
                  </li>
                ) : null}
            ...
            <div className="grid gap-2.5">
              ...
              <Button
                variant="outline"
                className="h-12 rounded-full border-2 border-primary bg-white font-bold text-black hover:bg-primary hover:text-primary-foreground"
