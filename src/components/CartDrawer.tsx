      ...
      <SheetContent className="flex w-full max-w-md flex-col p-0 sm:max-w-md">
        <SheetHeader className="border-b p-5">
          <div className="flex items-center justify-between gap-2 pr-8">
            <SheetTitle className="flex items-center gap-2 text-xl uppercase tracking-wide text-black">
        ...
        <div className="border-b bg-muted p-4">
          <p className="text-sm font-semibold">
            {remaining > 0 ? (
              <>
                Te faltan <b className="text-black">{precioMx(remaining)}</b> para envío gratis
              </>
            ) : (
            ...
            <div className="space-y-5">
              {lineas.map((linea) => (
                <div key={`${linea.paqueteId ?? "suelta"}-${linea.sku}`} className="flex gap-3 border-b pb-5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <b className="mt-1 block text-black">{precioMx(linea.precio)}</b>
              ...
              {recomendaciones.length ? (
                <section className="border-t pt-5" aria-label="Podría interesarte">
                  <h3 className="mb-3 text-sm font-extrabold uppercase tracking-wide text-black">Podría interesarte…</h3>
                  <ProductCarousel
                    ...
                    {recomendaciones.map((item) => (
                      <article key={item.sku} className="flex h-full gap-3 rounded-xl border bg-card p-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <b className="mt-2 block text-sm text-black">{precioMx(item.precio)} MXN</b>
                          <Button
                            variant="outline"
        ...
        {lineas.length > 0 ? (
          <div className="border-t p-5">
            ...
            <div className="mt-3 flex justify-between text-lg">
              <span>Subtotal</span>
              <b className="font-display text-black">{precioMx(total)}</b>
            </div>
            <Button
              ...
              onClick={() => {
                window.open(
                  `${WHATSAPP_URL}?text=${encodeURIComponent(
                    `Hola, quiero finalizar este pedido de Eléctrica Valdez (${piezas} piezas, ${precioMx(total)}).`
                  )}`,
                  "_blank"
                );
              }}
