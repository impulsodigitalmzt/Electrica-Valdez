...
export function SearchPage({ q, categoria, iluminacion }: Props) {
  ...
  useEffect(() => {
    const handle = window.setTimeout(() => {
      setCargando(true);
      void fetchCatalogo({ q: iluminacion ? "" : q, categoria, limit: 90 })
        .then((data) => {
          if (iluminacion) {
  ...
  const filtersPanel = (
    <div className="space-y-7">
      <div>
        <h3 className="font-display text-sm font-extrabold uppercase text-black">Tipo de luminario</h3>
          ...
          {TIPOS_LUMINARIO.map((item) => (
            <button
              ...
              className={`flex items-center justify-between border-b py-2.5 text-left text-sm font-semibold transition ${
                tipo === item.id ? "text-accent" : "text-muted-foreground hover:text-black"
              }`}
            >
              {item.label}
      ...
      <div>
        <h3 className="font-display text-sm font-extrabold uppercase text-black">Marca</h3>
          ...
          {MARCAS_ILUMINACION.map((brand) => (
            <button
              ...
              className={`flex items-center gap-2 border px-3 py-1.5 text-xs font-bold transition ${
                marcas.includes(brand) ? "border-primary bg-primary text-primary-foreground" : "text-muted-foreground hover:border-primary hover:text-black"
              }`}
            >
      ...
      <div>
        <h3 className="font-display text-sm font-extrabold uppercase text-black">Temperatura de color</h3>
      ...
      <div>
        <h3 className="font-display text-sm font-extrabold uppercase text-black">Uso</h3>
          ...
          {["todos", ...USOS].map((item) => (
            <button
              ...
              className={`flex-1 border px-3 py-2 text-xs font-bold capitalize transition ${
                uso === item ? "border-primary bg-primary text-primary-foreground" : "text-muted-foreground hover:border-primary hover:text-black"
              }`}
            >
              {item}
            </button>
      ...
      <div>
        <h3 className="font-display text-sm font-extrabold uppercase text-black">Precio máximo</h3>
        <input
          type="range"
          min={89}
          max={techoPrecio}
        ...
        />
        <p className="mt-2 text-sm font-bold text-black">Hasta {precioMx(Math.min(maxPrice, techoPrecio))}</p>
      </div>

      <label className="flex cursor-pointer items-center gap-3 border-t pt-5 text-sm font-semibold text-black">
        <input
          type="checkbox"
          checked={onlyOffers}
      ...
      <Button variant="outline" className="w-full border-primary text-black" onClick={resetFilters}>
    ...
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b pb-4">
              <p className="text-sm text-muted-foreground">
                <b className="text-black">{results.length}</b> productos encontrados
              </p>
              <div className="flex items-center gap-2">
                ...
                <label className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <select
                    ...
                    className="h-10 border bg-background px-3 text-sm font-semibold text-black outline-none focus:border-accent"
                  >
            ...
            {!cargando && results.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <h2 className="mt-4 text-xl font-bold text-black">Sin resultados</h2>
      ...
      <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
        <SheetContent side="left" className="w-[88%] overflow-y-auto">
          <SheetHeader>
            <SheetTitle className="text-left text-2xl text-black">Filtros</SheetTitle>
          </SheetHeader>
