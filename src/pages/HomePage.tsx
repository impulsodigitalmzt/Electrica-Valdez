...
const OFERTAS = DEMO_PRODUCTOS.filter((producto) => (producto.precioAnterior ?? 0) > producto.precio);
      ...
      <main id="inicio">
        ...
        <section className="mx-auto max-w-6xl px-4 py-10 sm:py-12" aria-labelledby="empresa">
          <div className="mx-auto mb-8 max-w-3xl text-center sm:mb-10">
            ...
            <h2 id="empresa" className="mt-1.5 text-xl font-extrabold leading-tight text-black sm:text-3xl">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 sm:gap-5 lg:gap-8">
            {SECTORES_TIENDA.map((item) => (
              <AppLink key={item.id} to={item.to} className="group block text-center">
                ...
                <span className="mt-3 block font-display text-lg font-extrabold text-black">
                  {item.label}
                </span>
        ...
        <section className="mx-auto max-w-7xl px-4 py-14 sm:py-16" aria-labelledby="colecciones">
          <div className="mb-8 text-center">
            ...
            <h2 id="colecciones" className="mt-2 text-2xl font-extrabold text-black sm:text-4xl">
          ...
          <div className="mx-auto max-w-7xl px-4">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-extrabold text-black sm:text-4xl">Iluminación para cada ambiente</h2>
              </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              {CATEGORY_TILES.map((item) => (
                <AppLink key={item.label} to={`/iluminacion?q=${encodeURIComponent(item.q.split(" ")[0])}`} className="group text-left">
                  ...
                  <span className="mt-3 flex items-center justify-between font-display text-sm font-bold text-black sm:text-base">
                    {item.label}
        ...
        <section className="mx-auto max-w-7xl px-4 pb-14" aria-labelledby="promos-sucursal">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              ...
              <h2 id="promos-sucursal" className="mt-2 text-2xl font-extrabold text-black sm:text-4xl">
                Lo que está en oferta
              </h2>
            </div>
            <Button variant="ghost" className="hidden text-black sm:flex" asChild>
              <AppLink to="/buscar?q=oferta">
                Ver ofertas <ArrowRight />
              </AppLink>
            </Button>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            {OFERTAS.slice(0, 8).map((producto) => (
              <ProductCard
                key={producto.sku}
                producto={producto}
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
            {PROMO_TILES.map((item) => (
              <AppLink key={item.label} to={item.to} className="group overflow-hidden border bg-card">
                <img src={item.image} alt={item.label} className="aspect-square w-full bg-background object-contain p-3 transition-transform duration-500 group-hover:scale-105" />
                <span className="block px-3 py-3 text-sm font-bold text-black">{item.label}</span>
              </AppLink>
            ))}
          </div>
        </section>
        ...
        <Brands />
        <Faq />
        <section className="bg-muted py-14 sm:py-16" aria-labelledby="servicio-domicilio">
          <div className="mx-auto max-w-5xl px-4">
            <div className="mb-6 text-center">
              <span className="text-xs font-bold uppercase text-accent">Envíos</span>
              <h2 id="servicio-domicilio" className="mt-2 text-2xl font-extrabold text-black sm:text-4xl">
                Servicio a domicilio
              </h2>
              <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
                Despachamos hasta la puerta de tu hogar. Pregunta por cobertura en Mazatlán, Culiacán y envíos a todo México.
              </p>
            </div>
            <a
              href={WHATSAPP_URL}
              target="_blank"
              rel="noreferrer"
              className="block overflow-hidden bg-card shadow-sm ring-1 ring-black/5"
              aria-label="Solicitar servicio a domicilio por WhatsApp"
            >
              <img
                src={SERVICIO_DOMICILIO}
                alt="Despachamos hasta la puerta de tu hogar. Servicio a domicilio de Eléctrica Valdez."
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div className="flex flex-col justify-center">
                <DialogHeader>
                  ...
                  <DialogTitle className="text-2xl leading-tight text-black">{quick.nombre}</DialogTitle>
                  <DialogDescription>
                    {quick.descripcion || "Producto original con garantía de fabricante. Disponibilidad inmediata."}
                  </DialogDescription>
                </DialogHeader>
                <div className="mt-5">
                  <b className="font-display text-3xl text-black">{precioMx(quick.precio)}</b>
                </div>
                <div className="mt-5 flex items-center gap-2 text-sm text-black">
                ...
                <Button variant="ghost" className="mt-2 text-black" onClick={() => navigate(`/producto/${encodeURIComponent(quick.sku)}`)}>
                  Ver ficha completa
                </Button>
              </div>
            </div>
      ...
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-extrabold text-black sm:text-4xl">{titulo}</h2>
          </div>
          <Button variant="ghost" className="hidden text-black sm:flex" asChild>
            <AppLink to={to}>
              Ver todo <ArrowRight />
