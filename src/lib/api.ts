...
export function placeholderProducto(producto: Pick<Producto, "nombre" | "categoria">): string {
  ...
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400"><rect fill="#f3f7f4" width="400" height="400"/><text x="50%" y="48%" text-anchor="middle" font-size="92" font-family="Barlow, sans-serif" fill="#0e7a3a">${letra}</text><text x="50%" y="68%" text-anchor="middle" font-size="18" font-family="Barlow, sans-serif" fill="#1a2e22">${producto.categoria}</text></svg>`
  )}`;
}
